  ...
  const searchSystemPrompt = [
    {
      ...
      text: [
        ...
        "4. [player name] position MLB roster need suitors [current year] — find which teams have a need at this player's position, who currently plays that position for likely suitor teams, and any reported interest",
        "",
        ...
        "PLAYER CONTEXT: Contract status, age, performance, position — from search only.",
        "ROSTER LANDSCAPE: For the player's position, which teams have a need? Who currently starts at that position for likely suitor teams? Use only what the search returns — do not guess.",
  ...
  const analysisSystemPrompt = [
    {
      ...
      text: [
        ...
        "1. ROSTER FIT — does the team have a confirmed need at this position per the findings? Do NOT suggest a team that already has an established player at that position unless the findings indicate otherwise.",
        ...
        "If no destination is named in the rumor, identify the most logical landing spots based on the player's position, contract, and which teams the findings show have a need.",
